...
class StrategyTab:
    ...
    def _build_q3(self, p):
        ...
        # 52周价格区间
        f4 = ttk.Frame(p); f4.pack(fill=tk.X, pady=2)
        self.vars['req_52w_high'] = tk.BooleanVar()
        ttk.Checkbutton(f4, text="距52周高点 ≤", variable=self.vars['req_52w_high']).pack(side=tk.LEFT)
        self.vars['p52w_high_pct'] = tk.DoubleVar(value=25.0)
        ttk.Entry(f4, textvariable=self.vars['p52w_high_pct'], width=5).pack(side=tk.LEFT, padx=2)
        ttk.Label(f4, text="%").pack(side=tk.LEFT, padx=(0,10))
        self.vars['req_52w_low'] = tk.BooleanVar()
        ttk.Checkbutton(f4, text="距52周低点 ≥", variable=self.vars['req_52w_low']).pack(side=tk.LEFT)
        self.vars['p52w_low_pct'] = tk.DoubleVar(value=0.0)
        ttk.Entry(f4, textvariable=self.vars['p52w_low_pct'], width=5).pack(side=tk.LEFT, padx=2)
        ttk.Label(f4, text="%").pack(side=tk.LEFT)
    ...
    def load_to_ui(self, p):
        ...
        # 52周价格区间
        p52f = p.get('price_distance_filters', {})
        self.vars['req_52w_high'].set(p52f.get('enable_52w_high', False))
        self.vars['p52w_high_pct'].set(p52f.get('pct_within_52w_high_max', 25.0))
        self.vars['req_52w_low'].set(p52f.get('enable_52w_low', False))
        self.vars['p52w_low_pct'].set(p52f.get('pct_above_52w_low_min', 0.0))
    ...
    def on_save(self):
        ...
        p['price_distance_filters'] = {
            'enable_52w_high': self.vars['req_52w_high'].get(),
            'pct_within_52w_high_max': self.vars['p52w_high_pct'].get(),
            'enable_52w_low': self.vars['req_52w_low'].get(),
            'pct_above_52w_low_min': self.vars['p52w_low_pct'].get()
        }
