...
#### 核心功能
*   **独立策略设置**：每个持仓可设置不同的止盈止损策略
*   **实时监控**：自动跟踪持仓的最高价，触发条件时返回操作建议
*   **支持多种策略类型**：
    - 止损：固定止损、移动止损(trailing)、ATR波动止损
    - 止盈：分批止盈、移动止盈、RSI超买预警、固定目标价

#### 数据库字段扩展（`positions`表）
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `stop_loss_type` | TEXT | 'trailing' | 止损类型: fixed/trailing/atr |
| `stop_loss_value` | REAL | 0.10 | 止损比例（10%表示从高点回落10%） |
| `profit_exit_type` | TEXT | 'tiered' | 止盈类型: trailing/tiered/rsi/fixed |
| `profit_exit_value` | REAL | 0.15 | 止盈比例 |
| `trailing_mode` | TEXT | 'strict' | 移动模式: strict(严格)/loose(宽松2%容忍) |
| `highest_since_buy` | REAL | 0 | 买入后最高价（自动跟踪） |
| `atr_multiplier` | REAL | 3.0 | ATR倍数（ATR止损用） |
| `tiered_profit_1` | REAL | 0.15 | 分批止盈第一阶（涨15%卖1/3） |
| `tiered_profit_2` | REAL | 0.25 | 分批止盈第二阶（涨25%再卖1/3） |
| `tiered_profit_3` | REAL | 0.40 | 分批止盈第三阶（涨40%后从高点回落8%全卖） |
| `rsi_overbought_level` | REAL | 80.0 | RSI超买预警线 |

#### 策略逻辑

**止损策略**：
- `fixed`：价格跌到 `stop_loss_price` 触发卖出
- `trailing`：从 `highest_since_buy` 回落 `stop_loss_value*100%` 触发卖出
- `atr`：从 `highest_since_buy` 回落 `atr_multiplier * ATR` 触发卖出

**止盈策略**：
- `tiered`（推荐）：分3批止盈
  - 涨15%卖1/3
  - 涨25%再卖1/3
  - 涨40%后从高点回落8%全卖（让利润奔跑）
- `trailing`：从最高点回落 `profit_exit_value*100%` 触发卖出
- `rsi`：RSI超过 `rsi_overbought_level` 预警
- `fixed`：价格达到 `target_price` 触发卖出
